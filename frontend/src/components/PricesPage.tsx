...
export function PricesPage(props: PricesPageProps) {
  ...
  const filtered = useMemo(() => props.items.filter(item => {
    if (search && !item.model_name.toLowerCase().includes(search.trim().toLowerCase())) return false;
    if (scope === 'global') return item.provider_id === null;
    return !providerId || item.provider_id === Number(providerId);
  }).sort((a, b) => a.model_name.localeCompare(b.model_name) || (a.provider_id ?? -1) - (b.provider_id ?? -1)), [props.items, search, scope, providerId]);
  ...
  return <Box sx={{ display: 'grid', gap: 2, minWidth: 0 }}>
    <FilterBar primary={<>
      <InputBase value={search} placeholder={t('搜索模型价格')} sx={{ gridColumn: { xl: 'span 3' } }} inputProps={{ 'aria-label': t('搜索模型价格') }} startAdornment={<Search size={16} />} onChange={event => filter('q', event.target.value, true)} />
    ...
    {visible.length > 0 ? <TableContainer><Table className="table-fixed" size="small" aria-label={t('当前可用价格项')} sx={{ minWidth: 1150, '& td': { overflow: 'hidden' } }}>
      ...
      <TableBody>{visible.map(item => <TableRow key={item.id} hover>
        <TableCell className="whitespace-nowrap"><Box className="block truncate" title={item.model_name} sx={{ fontFamily: 'monospace' }}>{item.model_name}</Box></TableCell>
