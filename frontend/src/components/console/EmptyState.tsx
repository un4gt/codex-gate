...
export function EmptyState(props: { title: string; description?: string; action?: JSX.Element }) {
  return (
    <div class="flex flex-col items-center justify-center border border-dashed border-border/60 bg-muted/10 p-10 text-center">
      ...
      <h3 class="mb-2 font-mono text-sm font-medium uppercase tracking-widest text-foreground">{t(props.title)}</h3>
      <Show when={props.description}>
        <p class="max-w-md font-mono text-[0.65rem] uppercase tracking-widest text-muted-foreground opacity-70">{t(props.description!)}</p>
      </Show>
