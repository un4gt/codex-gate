...
export function EmptyState(props: { title: string; description?: string; action?: JSX.Element }) {
  return (
    <div class="flex flex-col items-center justify-center p-12 border border-dashed border-border/60 bg-transparent text-center">
      ...
      <h3 class="text-sm font-medium tracking-widest uppercase font-mono text-foreground mb-2">{props.title}</h3>
      <Show when={props.description}>
        <p class="text-[0.65rem] font-mono tracking-widest uppercase text-muted-foreground opacity-60 max-w-md">{props.description}</p>
      </Show>
